...
import { Brain, Mic, MicOff, Sparkles, ChevronDown, ChevronUp, Link2, Search, X, Heart } from "lucide-react";
...
export default function SecondBrainTab({ userCodeId }: { userCodeId: string }) {
  ...
  const analyzeThought = async (thought: Thought) => {
    ...
    try {
      const recentForContext = thoughts.slice(0, 10).map(t => ({
        type: t.type,
        content: t.content,
      }));
      ...
      setThoughts(prev =>
        prev.map(t =>
          t.id === thought.id
            ? { ...t, analysis, type: selectedType === "auto" ? detectedType : t.type, keywords, emotion_valence: valence, emotion_intensity: intensity }
            : t
        )
      );

      // Auto-connect by keywords
      if (keywords.length > 0) {
        findAndSaveConnections(thought.id, keywords);
      }
    } catch (e) {
  ...
  const findAndSaveConnections = async (thoughtId: string, keywords: string[]) => {
    ...
    if (connected && connected.length > 0) {
      const connections = connected.map(c => ({
        user_code_id: userCodeId,
        thought_a: thoughtId,
        thought_b: c.id,
      ...
      await supabase.from("thought_connections").upsert(connections, { onConflict: "thought_a,thought_b" });
    }
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto px-4 py-4 lg:py-6 pb-28">
      {/* Header */}
      <div className="flex items-center gap-2 mb-4">
        <Brain size={20} className="text-primary" />
        <h2 className="text-base font-bold font-display text-foreground tracking-wide">Segundo Cérebro</h2>
      </div>
      ...
      <div className="rounded-2xl p-4" style={{ background: "hsl(var(--card))", border: "1px solid hsl(var(--border))" }}>
        ...
        <div className="flex flex-wrap gap-1.5 mt-3 mb-3">
          <button
            ...
            style={{
              background: selectedType === "auto" ? "hsl(var(--primary) / 0.15)" : "transparent",
              color: selectedType === "auto" ? "hsl(var(--primary))" : "hsl(var(--muted-foreground))",
              border: selectedType === "auto" ? "1px solid hsl(var(--primary) / 0.3)" : "1px solid hsl(var(--border))",
            }}
          >
            ✨ Auto
          </button>
          {THOUGHT_TYPES.map(t => (
            <button
              key={t.key}
              onClick={() => { setSelectedType(t.key); haptic("light"); }}
              className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider transition-all"
              style={{
                background: selectedType === t.key ? `${t.color}22` : "transparent",
                color: selectedType === t.key ? t.color : "hsl(var(--muted-foreground))",
                border: selectedType === t.key ? `1px solid ${t.color}44` : "1px solid hsl(var(--border))",
              }}
            >
              {t.emoji} {t.label}
            </button>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            style={{
              background: isListening ? "hsl(var(--destructive) / 0.15)" : "hsl(var(--muted) / 0.3)",
              color: isListening ? "hsl(var(--destructive))" : "hsl(var(--muted-foreground))",
            }}
          >
          ...
          <button
            ...
            disabled={!content.trim() || isRegistering}
      ...
      <div className="flex items-center justify-between mt-6 mb-3">
        <h3 className="text-xs font-bold uppercase tracking-[2px] text-muted-foreground/60 font-ui">
...
function ThoughtCard({
  ...
  return (
    ...
    >
      <div className="p-3.5">
        ...
        <div className="flex items-center justify-between mb-1.5">
          ...
          <div className="flex items-center gap-1">
            {isAnalyzing && (
              <span className="text-[10px] text-primary animate-pulse font-ui">Analisando...</span>
            )}
        ...
        {/* Content preview */}
        <p className={`text-sm text-foreground/80 font-body leading-relaxed ${!isExpanded ? "line-clamp-2" : ""}`}>
          {thought.content}
        </p>

        {/* Analysis preview (collapsed) */}
        {!isExpanded && analysis?.diagnosis?.summary && (
          <p className="text-[11px] text-muted-foreground/60 mt-1.5 italic font-body line-clamp-1">
      ...
      {isExpanded && analysis && (
        <div
          className="px-3.5 pb-4 pt-1 space-y-3 animate-fade-in"
        ...
        >
          ...
          {analysis.psychological_analysis && (
            <div>
              ...
              <p className="text-xs text-foreground/70 font-body">
                <strong style={{ color: typeInfo.color }}>{analysis.psychological_analysis.pattern}</strong>
              </p>
              <p className="text-xs text-muted-foreground/70 font-body mt-0.5">{analysis.psychological_analysis.explanation}</p>
              <p className="text-xs italic mt-1 font-body" style={{ color: "#8b9e7a" }}>
          ...
          {analysis.biblical_analysis && (
            <div>
              ...
              <p className="text-xs text-foreground/70 font-body">
                <strong className="text-primary">{analysis.biblical_analysis.principle}</strong>
              </p>
          ...
          {analysis.diagnosis && (
            <div>
              ...
              <p className="text-xs mt-1 font-body" style={{ color: "#8b9e7a" }}>
                ⚡ {analysis.diagnosis.action}
              </p>
              <p className="text-xs mt-1 italic font-body text-primary/70">
