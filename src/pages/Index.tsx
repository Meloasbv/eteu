...
export default function BiblePlan() {
  ...
  const weekProg = useCallback((wi: number) => {
    let total = 0, done = 0;
    WEEKS[wi].days.forEach((d, di) => {
      if (d.r.length) { total++; if (checked[`${wi}-${di}`]) done++; }
    });
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "32px 24px 20px", textAlign: "center", borderBottom: "1px solid rgba(200,180,140,.08)" }}>
        ...
        <h1 style={{ fontSize: "clamp(26px,5vw,38px)", fontWeight: 300, color: "#e8d8b8", letterSpacing: 1, marginBottom: 20 }}>
          Leitura Bíblica Cronológica
        </h1>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16 }}>
          <svg width="52" height="52" viewBox="0 0 52 52" style={{ transform: "rotate(-90deg)" }}>
            <defs>
              <linearGradient id="pg" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stopColor="#C8A55C"/>
            ...
            <circle cx="26" cy="26" r="22" fill="none" stroke="url(#pg)" strokeWidth="3"
              strokeDasharray={`${prog * circumference} ${circumference}`}
              strokeLinecap="round"
      ...
      <div style={{ padding: "20px 24px 8px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 80, height: 4, background: "rgba(200,180,140,.1)", borderRadius: 2, overflow: "hidden" }}>
            <div style={{
              width: `${wp * 100}%`, height: "100%", borderRadius: 2,
      ...
      <div style={{ padding: "4px 24px 12px", display: "flex", gap: 8 }}>
        {([[-1,"‹"],[1,"›"]] as [number, string][]).map(([delta, label]) => (
          <button key={label} onClick={() => setActiveWeek(w => Math.max(0, Math.min(WEEKS.length - 1, w + delta)))}
            ...
            style={{
              ...
              color: "#a09078", cursor: "pointer", fontSize: 18, display: "flex",
          ...
          return (
            <div key={di} onClick={() => toggle(activeWeek, di)} style={{
              ...
              <div style={{
                ...
                opacity: isDone ? 0.7 : 0.5, borderRadius: "14px 14px 0 0",
              }}/>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 17, fontWeight: 600, color: isDone ? "#6B8E6B" : c, letterSpacing: 0.5 }}>
                    {ABBREVS[di]}
                  </span>
                  <span style={{ fontSize: 12, color: "#6a5a48" }}>
